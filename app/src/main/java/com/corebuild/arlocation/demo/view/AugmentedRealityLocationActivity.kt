...
class AugmentedRealityLocationActivity : AppCompatActivity(), Callback<VenueWrapper> {
    ...
    private fun setupRetrofit() {
        apiQueryParams["client_id"] = "yourAPIKEY"
        apiQueryParams["client_secret"] = "yourAPISECRET"
        apiQueryParams["v"] = "20200608"
    ...
    private fun setupAndRenderVenuesMarkers() {
        venuesSet.forEach { venue ->
            val completableFutureViewRenderable = ViewRenderable.builder()
                .setView(this, R.layout.location_layout_rederable_new)
                .build()
...
//        nodeLayout.categoryIcon.background.setColorFilter(
