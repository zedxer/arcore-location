...
class VenueTypeConverter : JsonDeserializer<VenueWrapper> {
    ...
    private fun getVenue(rawVenue: JsonObject): Venue {
        ...
        //TODO IN THE RESPONSE WE ARE GETTING IN A NULL OBJECT IN CATEGORIES.
        var iconURL = ""
        try {
            if (rawVenue.getAsJsonArray("categories").toList().isNotEmpty()) {
                val categories = rawVenue.getAsJsonArray("categories")?.get(0)?.asJsonObject
                iconURL = categories?.getAsJsonObject("icon")
                    ?.getAsJsonPrimitive("prefix")?.asString + "64.png"
            }
